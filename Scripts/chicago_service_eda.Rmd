...
chicago_citizen$Citizen <- if_else(
  chicago_citizen$Citizen == "Foreign born", "No", "Yes")
```
...
chicago_updated_covid <- read_csv("chicago_updated_cases.csv")
chicago_updated_covid <- chicago_updated_covid %>% 
  mutate(case_rate = Cases/Population*100000) %>% 
  mutate(pos_rate = Cases/Tested)

chicago_updated_covid$Zip <- as.factor(chicago_updated_covid$Zip)

chicago_ind_by_class <- left_join(chicago_ind_by_class, chicago_updated_covid, by = c("GEOID" = "Zip"))
...
chicago_sex_by_age <- left_join(chicago_sex_by_age, chicago_updated_covid, by = c("GEOID" = "Zip"))
