...
# creating new_cases and 7 day moving average metric, and 3/7/30 day sums.
final_covid <- final_covid %>% 
  ...
  mutate(new_cases = diff(c(0,cases)),
         ...
         sum_3_days = roll_sum(cases, n = 3, align = "right", fill = NA),
         sum_7_days = roll_sum(cases, n = 7, align = "right", fill = NA),
         sum_30_days = roll_sum(cases, n = 30, align = "right", fill = NA),
         is_hotspot = ifelse((sum_7_days > 100) & (sum_7_days - lag(sum_7_days) > 0) &
                            ((lag(sum_3_days)-sum_3_days)/lag(sum_3_days) < .6 | sum_3_days - lag(sum_3_days) > 0) &                           
                            (sum_7_days/sum_30_days > .31) &
                            ((sum_3_days-lag(sum_3_days))/lag(sum_3_days) > .6 | (sum_7_days-lag(sum_7_days))/lag(sum_7_days) > .6), "Yes","No"))
final_covid$is_hotspot[is.na(final_covid$is_hotspot)] <- "No"
final_covid$is_hotspot <- as.integer(as.character(final_covid$is_hotspot) == "Yes")
