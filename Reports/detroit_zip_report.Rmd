...
When we test our correlation, we get a value of `r cor.test(det_occupation_nonessen_cor_death$prop_nonessen, det_occupation_nonessen_cor_death$Cases_per_HundThou, use = "complete.obs")$estimate` ($p=$ `r cor.test(det_occupation_nonessen_cor_death$prop_nonessen, det_occupation_nonessen_cor_death$Cases_per_HundThou, use = "complete.obs")$p.value`). This tells us that there is a moderate correlation between the percentage of the population working "nonessential" jobs and the rate of COVID deaths: as a ZIP code has more nonessential workers, it also has a higher COVID death rate.
...
```{r}
det_insurance <- get_acs(geography = "zcta",
                           table = "B18135") %>% 
  filter(GEOID %in% zip_code_det$zip,
         !variable %in% c("B18135_023"))
det_insurance <- left_join(det_insurance, variables_2018[, 1:2], by = "variable")
det_insurance$label <- as_factor(str_replace(det_insurance$label, ".*!!(.*)", "\\1"))
```

Analyze private health insurance coverage
```{r}
det_insurance_private <- det_insurance %>% 
  group_by(GEOID) %>% 
  mutate(n = sum(estimate),
         prop = estimate/n) %>% 
...
lvls <- det_insurance_private %>% 
  arrange(prop_priv) %>% 
  pull(GEOID)

det_insurance <- left_join(det_insurance, detroit_covid, by = c("GEOID" = "ZIP"))

ggplot(det_insurance) +
  geom_col(aes(factor(GEOID, levels = lvls), estimate, fill = label),
           position = "fill") +
  theme(axis.text.x = element_text(angle = 45,
                                   hjust = 1)) +
...
```{r}
det_private_cor <- left_join(det_insurance_private, detroit_covid, by = c("GEOID" = "ZIP"))
ggplot(det_private_cor) +
  geom_point(aes(x = prop_priv, y = Cases_per_HundThou)) +
...
```{r}
private_cor<-cor.test(det_private_cor$prop_priv, det_private_cor$Cases_per_HundThou, method="pearson")
...
```{r}
det_insurance_public <- det_insurance %>% 
  group_by(GEOID) %>% 
  mutate(n = sum(estimate),
         prop = estimate/n) %>% 
...
```{r}
det_public_cor <- left_join(det_insurance_public, detroit_covid, by = c("GEOID" = "ZIP"))
ggplot(det_public_cor) +
  geom_point(aes(x = prop_public, y = Cases_per_HundThou)) +
...
```{r}
public_cor<-cor.test(det_public_cor$prop_public, det_public_cor$Cases_per_HundThou, method= "pearson")
public_cor$p.value
public_cor$estimate
...
det_transport <- left_join(det_transport, detroit_covid, by = c("GEOID" = "ZIP"))
