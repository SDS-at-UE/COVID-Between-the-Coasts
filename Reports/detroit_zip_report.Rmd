...
det_gini <- filter(det_gini, GEOID != '55450')
...
```{r}
cor.test(det_gini$estimate, det_gini$case_rate, use = "complete.obs")
```
...
```{r}
cor.test(det_private_cor$prop_priv, det_private_cor$case_rate, use = "complete.obs")
```
...
```{r}
cor.test(det_public_cor$prop_public, det_public_cor$case_rate, use = "complete.obs")
```
...
```{r}
det_trans_cor <- left_join(det_transport_public, detroit_covid, by = c("GEOID"="ZIP"))
cor.test(det_trans_cor$prop_pub, det_trans_cor$case_rate, use = "complete.obs")
```
