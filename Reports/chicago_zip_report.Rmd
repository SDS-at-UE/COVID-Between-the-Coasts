...
COVID data for zip code
```{r}
chicago_income <- left_join(chicago_trans, chicago_covid, by = c("GEOID" = "Zip"))
```


# Health Insurance
```{r}
chicago_ins <- get_acs(geography = "zcta",
                          year = 2018,
                          table = "B18135") %>% 
  filter(GEOID %in% zip_code_chicago$zip,
         !variable %in% c("B18135_001"))
chicago_ins <- left_join(chicago_ins, variables_2018[, 1:2], by = "variable")
chicago_ins$label <- as_factor(str_replace(chicago_ins$label, ".*!!(.*)", "\\1"))
...
Analyzing Private Health Insurance Coverage Data
```{r}
chicago_private_ins <- chicago_ins %>% 
  group_by(GEOID) %>% 
  mutate(n = sum(estimate),
         prop = estimate/n) %>% 
  filter(label %in% c("With private health insurance coverage")) %>% 
  mutate(prop_priv_ins = sum(prop)) %>% 
  dplyr::select(GEOID, prop_priv_ins) %>% 
  distinct(GEOID, prop_priv_ins)

lvls <- chicago_private_ins %>% 
  arrange(prop_priv_ins) %>% 
  pull(GEOID)
...
ggplot(chicago_ins) +
  ...
  theme(axis.text.x = element_text(angle = 45,
                                   hjust = 1))+
labs(title = "Health Insurance Coverage Distribution for Each Chicago Zip Code",
       x = "Zip Code",
       y = "Proportion", 
       fill = "Health Insurance Coverage")
```

Test Correlation between percentage of population who have private health insurance coverage to the case rate of COVID
```{r}
chicago_ins_cor <- left_join(chicago_private_ins, chicago_covid, by = c("GEOID"="Zip"))
cor.test(chicago_ins_cor$prop_priv_ins, chicago_ins_cor$case_rate, use = "complete.obs")
```
There is a negative correlation calculated when looking at individuals with private health insurance and COVID. That correlation is -0.44.

Analyzing Public Health Insurance Coverage Data
```{r}
chicago_public_ins <- chicago_ins %>% 
  group_by(GEOID) %>% 
  mutate(n = sum(estimate),
         prop = estimate/n) %>% 
  filter(label %in% c("With public health coverage")) %>% 
  mutate(prop_pub_ins = sum(prop)) %>% 
  dplyr::select(GEOID, prop_pub_ins) %>% 
  distinct(GEOID, prop_pub_ins)

lvls <- chicago_public_ins %>% 
  arrange(prop_pub_ins) %>% 
  pull(GEOID)
...
Test Correlation between percentage of population who have public health insurance coverage to the case rate of COVID
```{r}
chicago_ins_cor <- left_join(chicago_public_ins, chicago_covid, by = c("GEOID"="Zip"))
cor.test(chicago_ins_cor$prop_pub_ins, chicago_ins_cor$case_rate, use = "complete.obs")
```
There is a positive correlation of 0.37 between those with public health insurance and COVID cases. 

COVID data for zip code
```{r}
chicago_ins <- left_join(chicago_ins, chicago_covid, by = c("GEOID" = "Zip"))
```


# Conclusions
We analyzed 8 categories and 12 variables throughout the zip codes in Chicago. We saw two variables stick out as they crossed over 0.5. Half of the variables were positively correlated when the other half were negatively correlated. Essential workers and non essential workers saw some higher correlations numbers attached to them. A couple variables that I thought would definitely have correlation in favor of COVID cases did not show any correlations including age and race. I think we could dig into these two variables more and see if we could find other ranges in those categories to show some correlation. For public transportation, I expected a high positive number, but the results showed a high negative number. Are there underlying reasons why people who use public transportation are getting less sick? Individuals who are not citizens are seeing higher rates of COVID-19. Are they able to get the help they need with new government restrictions in place? I'm sure we can dig and find some more insight on these factors to see as to why they have the correlation they do.
