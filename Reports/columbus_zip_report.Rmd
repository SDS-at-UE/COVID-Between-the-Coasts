...
Public transportation is typically used by those with lower incomes. This means of transportation would seem to have a higher chance of contracting COVID. Is our hypothesis true?
...
The percentage of the population that uses public transportation is not too high, never reaching above 17\%. Most Columbus residents provide their own motorized transportation, while in a few ZIP codes walking and working from home is quite popular. It's relationship to COVID can be seen in the following graph. 
...
ggplot(col_trans_public) +
  geom_point(aes(x = prop_public, y = average)) +
...
ggplot(col_occ) +
  ...
  labs(title = "Occupation Distribution for Each Columbus ZIP Code",
       ...
       fill = "Occupation \nType")
```
Our essential occupations were said to be in two categories: Natural resources, construction, and maintenance; and production, transportation, and material moving. The percentage of the population that has these occupations is not the majority, never reaching above 40\%. Most Columbus residents worked from home with their nonessential jobs. The relationship between essential occupations and COVID is illustrated next.


```{r}
ggplot(col_occ_ess) +
  geom_point(aes(x = prop_ess, y = average)) +
  labs(title = "Moderate Relationship Between Essential Occupations and COVID",
       x = "Proportion of People Who Have Essential Occupations",
       y = "Case Rate (per 100,000)")
```
...
Our test gives outputs of 0.00974 for our p-value (significant), R^2 equal to 0.39, and correlation of 0.6245. We see that essential occupations tend to have more cases of COVID-19.
...
ggplot(col_citizen) +
 ...
 legend.position = "bottom") +
  labs(title = "Citizenship Distribution for Each Columbus ZIP Code",
       ...
       fill = "Citizenship \nType")
```
The percentage of the population that is U.S. citizens is not less 75\% with an exception of one zipcode where that rate falls slightly. Let's explore the relationship to COVID in the graph below.


```{r}
col_citizen_prop_us <- left_join(col_citizen_prop_us, col_covid, by = c("GEOID" = "ZIP"))

ggplot(col_citizen_prop_us) +
  geom_point(aes(x = prop_us_citizen, y = average)) +
  labs(title = "Slight Relationship Between U.S. Citizens and COVID",
       x = "Proportion of People Who Are U.S. Citizens",
       y = "Case Rate (per 100,000)")
```
...
We have a slight positive correlation between U.S. citizens and COVID-19 cases. That correlation is 0.1997. With that being said, our p-value is insignificant at 0.894. There seems to be no real relation between these two factors.
...
ggplot(col_race) +
  geom_point(aes(x = prop, y = average)) +
  labs(title = "The White Population to COVID",
       x = "Proportion of White People",
       y = "Case Rate (per 100,000)")
```
